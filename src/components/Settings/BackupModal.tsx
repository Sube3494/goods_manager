...
export function BackupModal({ isOpen, onClose, type, file, requirePassword = true, onAction }: BackupModalProps) {
  ...
  const isExportWithOptionalPassword = type === "export" && !requirePassword;
  ...
  const handleConfirm = async () => {
    ...
    if (isExportWithOptionalPassword && password.length > 0 && password.length < 6) {
      setError("密码至少需要 6 位字符");
      return;
    }
                      ...
                      <p className="mt-4 text-[11px] text-muted-foreground leading-relaxed">
                        {type === "export"
                          ? requirePassword
                            ...
                            : "留空导出时，系统会使用内置备份密钥；填写自定义密码时，请务必牢记。"
                          : requirePassword
