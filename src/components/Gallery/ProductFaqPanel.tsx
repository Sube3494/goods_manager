...
export function ProductFaqPanel({ showBackLink = true, compactHeader = false }: ProductFaqPanelProps) {
  ...
  const flattenedRows = useMemo<FlattenedFaqRow[]>(() => {
    ...
    const sorted = [...items].sort((left, right) => {
      if (sortBy === "created-asc") return toTime(left.createdAt) - toTime(right.createdAt);
      if (sortBy === "question-asc") return getQuestion(left).localeCompare(getQuestion(right), "zh-CN", { sensitivity: "base", numeric: true });
      if (sortBy === "question-desc") return getQuestion(right).localeCompare(getQuestion(left), "zh-CN", { sensitivity: "base", numeric: true });
  ...
  const handleSelectProducts = (products: Product[]) => {
    ...
    for (const product of products) {
      if (!product.id || nextIds.has(product.id)) continue;
    ...
    <div className={cn("min-w-0 max-w-full space-y-5", compactHeader ? "md:space-y-5" : "md:space-y-7")}>
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <h1 className={cn("truncate font-semibold tracking-tight text-foreground", compactHeader ? "text-2xl sm:text-3xl" : "text-3xl sm:text-4xl")}>
                常见<span className="text-primary">问题</span>
              </h1>
              <p className={cn("mt-1 truncate text-sm text-muted-foreground", !compactHeader && "md:text-lg")}>集中整理客户常问内容，统一关联到相册商品</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex w-full flex-row items-center gap-2 transition-all">
        <div className="relative flex h-10 min-w-0 flex-1 items-center gap-2 rounded-full border border-border bg-white px-3 transition-all focus-within:ring-2 focus-within:ring-primary/20 dark:border-white/10 dark:bg-white/5 dark:hover:bg-white/10 sm:h-11 sm:gap-3 sm:px-5">
          <Search size={16} className="shrink-0 text-muted-foreground sm:h-[18px] sm:w-[18px]" />
          <input
            type="text"
            value={query}
            onChange={(event) => setQuery(event.target.value)}
            placeholder="搜索问题或答案..."
            className="h-full w-full border-none bg-transparent pr-8 text-sm text-foreground outline-none placeholder:text-muted-foreground"
          />
          {query && (
            <button
              onClick={() => setQuery("")}
              className="absolute right-4 top-1/2 rounded-full p-1 text-muted-foreground transition-colors hover:bg-black/5 hover:text-foreground dark:hover:bg-white/10"
              title="清空搜索"
            >
              <X size={14} />
            </button>
      ...
      {activeDraft && isMounted
        ? createPortal(
            <div className="fixed inset-0 z-[100000] flex items-center justify-center p-4 sm:p-6">
              <button type="button" aria-label="关闭编辑弹窗" onClick={() => setActiveDraft(null)} className="absolute inset-0 bg-[rgba(3,6,14,0.72)] backdrop-blur-md" />
              <section className="relative z-10 flex max-h-[calc(100vh-2rem)] w-full max-w-3xl flex-col overflow-hidden rounded-[24px] border border-slate-200/90 bg-[linear-gradient(180deg,#ffffff,#f8fbff)] shadow-[0_28px_90px_rgba(148,163,184,0.28)] dark:border-white/[0.08] dark:bg-[linear-gradient(180deg,rgba(9,14,24,0.985),rgba(11,17,28,0.975))] dark:shadow-[0_28px_90px_rgba(2,6,23,0.52)]">
                <div className="flex items-start justify-between gap-4 border-b border-slate-200/80 bg-[linear-gradient(90deg,rgba(255,255,255,0.96),rgba(59,130,246,0.06),transparent_55%)] p-4 dark:border-white/[0.06] dark:bg-[linear-gradient(90deg,rgba(255,255,255,0.03),rgba(59,130,246,0.04),transparent_55%)] sm:p-5">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">一条记录就是一问一答，可关联多个商品。</p>
                  </div>
                  <button type="button" onClick={() => setActiveDraft(null)} className="inline-flex h-9 w-9 items-center justify-center rounded-full text-muted-foreground transition-all hover:bg-slate-100 hover:text-foreground dark:hover:bg-white/8" title="关闭">
                    <X size={16} />
                  </button>
                </div>
                ...
                <div className="custom-scrollbar space-y-4 overflow-y-auto p-4 sm:p-5">
                  <div className="rounded-2xl border border-slate-200 bg-white/90 p-3 shadow-[0_10px_24px_rgba(148,163,184,0.12)] dark:border-white/10 dark:bg-white/5 dark:shadow-none">
                    <div className="mb-3 inline-flex items-center rounded-full border border-sky-200 bg-sky-50 px-2.5 py-1 text-sm font-medium text-sky-700 dark:border-sky-300/20 dark:bg-sky-300/10 dark:text-sky-100">
                      问题
                    </div>
                    <input
                  ...
                  <div className="rounded-2xl border border-slate-200 bg-white/90 p-3 shadow-[0_10px_24px_rgba(148,163,184,0.12)] dark:border-white/10 dark:bg-white/5 dark:shadow-none">
                    <div className="mb-3 flex items-center justify-between gap-3">
                      ...
                      <button onClick={() => setPickerOpen(true)} className="inline-flex h-8 items-center justify-center gap-2 rounded-full border border-slate-200 bg-white px-3 text-sm font-medium text-slate-700 transition-all hover:bg-slate-50 active:scale-95 dark:border-white/10 dark:bg-white/6 dark:text-foreground dark:hover:bg-white/10">
                        <Plus size={16} />
                        选择商品
                      </button>
                    </div>
                        ...
                        {activeDraft.products.map((product) => (
                          <div key={product.id} className="flex min-w-0 items-center gap-2.5 rounded-xl border border-slate-200 bg-white p-2 shadow-[0_8px_20px_rgba(148,163,184,0.1)] dark:border-white/10 dark:bg-white/5 dark:shadow-none">
                            <div className="relative flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-muted">
                              {product.image ? <Image src={product.image} alt={product.name} fill sizes="36px" className="object-cover" /> : <Package size={16} className="text-muted-foreground" />}
                            </div>
                            <div className="min-w-0 flex-1">
                            ...
                            <button onClick={() => removeProduct(product.id)} className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-muted-foreground transition-all hover:bg-red-500/10 hover:text-red-500" title="取消关联">
                              <X size={14} />
                            </button>
                          </div>
                        ))}
                ...
                <div className="flex flex-col-reverse gap-2 border-t border-slate-200/80 bg-slate-50/80 p-4 dark:border-white/[0.06] dark:bg-[rgba(255,255,255,0.015)] sm:flex-row sm:justify-end sm:p-5">
                  <button onClick={() => setActiveDraft(null)} className="h-10 w-full rounded-full border border-slate-200 bg-white px-4 text-sm font-medium text-slate-700 transition-all hover:bg-slate-50 active:scale-95 dark:border-white/10 dark:bg-white/6 dark:text-foreground dark:hover:bg-white/10 sm:w-auto">
                    取消
                  </button>
                  <button
                    onClick={handleSave}
      ...
      {isLoading ? (
        ...
        <div className="grid min-w-0 gap-4">
          {flattenedRows.map((row, index) => {
            const parentItem = items.find((item) => item.id === row.parentId);
            return (
              <section key={row.rowId} className="min-w-0 overflow-hidden rounded-[20px] border border-slate-200/90 bg-white/92 shadow-[0_18px_42px_rgba(148,163,184,0.18)] transition-all duration-200 hover:border-slate-300 dark:border-white/10 dark:bg-white/5 dark:shadow-[0_10px_24px_rgba(2,6,23,0.16)] dark:hover:border-white/10">
                <div className="flex w-full min-w-0 flex-col gap-2.5 bg-[linear-gradient(90deg,rgba(59,130,246,0.035),transparent_55%)] px-4 py-3.5 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
                  <div className="min-w-0 flex-1">
                    <div className="flex min-w-0 flex-col gap-2.5 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-start gap-2.5">
                          <div className="mt-0.5 inline-flex h-6 min-w-6 shrink-0 items-center justify-center rounded-full border border-sky-200 bg-sky-50 px-1.5 text-[11px] font-semibold text-sky-700 shadow-[0_6px_16px_rgba(56,189,248,0.12)] dark:border-sky-300/20 dark:bg-sky-300/10 dark:text-sky-100">
                            {index + 1}
                          </div>
                          <h2 className="min-w-0 flex-1 break-words text-[17px] font-medium leading-6 text-foreground sm:truncate">{row.entry.question || "未命名问题"}</h2>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between gap-3 sm:w-auto sm:justify-end">
                        {row.products.length > 0 ? (
                          <div className="flex min-w-0 items-center gap-2">
                            ...
                            <span className="truncate text-[11px] text-slate-500 dark:text-slate-400 sm:hidden">已关联 {row.products.length} 个商品</span>
                          </div>
                        ) : (
                        ...
                        <div className="flex shrink-0 items-center gap-1 rounded-full border border-slate-200 bg-slate-50/90 px-1 py-1 dark:border-white/10 dark:bg-white/[0.04]">
                          <button type="button" onClick={() => void handleCopyAnswer(row.entry.answer || "")} className="inline-flex h-8 w-8 items-center justify-center rounded-full text-slate-400 transition-all hover:bg-slate-100 hover:text-foreground active:scale-95 dark:text-slate-300/78 dark:hover:bg-white/[0.06]" title="复制答案">
                            <Copy size={14} />
                          </button>
                          {row.canEdit && parentItem && (
                            <>
                              <button
                                type="button"
                                onClick={() => setActiveDraft(toSingleEntryDraft(parentItem, row.entry.id))}
                              ...
                              <button
                                type="button"
                                onClick={() => void handleDelete(parentItem)}
                    ...
                    <div className="mt-3 rounded-2xl border border-slate-200 bg-slate-50/90 px-3.5 py-2.5 dark:border-white/10 dark:bg-white/5">
                      ...
                      <div className="mt-1.5 whitespace-pre-wrap text-sm leading-6 text-slate-700 dark:text-slate-300/82">{row.entry.answer || "暂未填写标准答案"}</div>
                    </div>

                    {row.products.length > 0 && (
                      <div className="mt-3 flex flex-wrap gap-2">
                        {row.products.map((product) => (
                          <span key={product.id} className="inline-flex items-center gap-1 rounded-full border border-slate-200 bg-white px-3 py-1 text-xs text-slate-600 dark:border-white/10 dark:bg-white/5 dark:text-slate-300">
                            {product.name}
