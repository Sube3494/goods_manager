...
export default function ProfilePage() {
  ...
  const buildProfileSnapshot = (nextName: string) =>
    JSON.stringify({
      name: nextName,
    });
  ...
  const saveProfile = async (nextName: string, silent = true) => {
  ...
  return (
    <div className="relative min-h-[calc(100dvh-4rem)] px-2.5 py-4 sm:px-6 sm:py-5 lg:px-8 animate-in fade-in zoom-in-95 duration-500">
            ...
            <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_auto] xl:items-start">
              <div className="flex items-start gap-3 sm:gap-4">
                ...
                <div className="min-w-0 flex-1 pt-0.5">
                  ...
                  <p className="mt-2 text-sm leading-relaxed text-muted-foreground">集中管理您的显示名称、登录邮箱及账号登录密码。</p>
                </div>
              </div>
        ...
        <motion.section id="profile-core" initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="overflow-hidden rounded-[22px] border border-border/70 bg-white/86 shadow-xl shadow-black/5 backdrop-blur-xl dark:bg-[#0b111e]/80 dark:shadow-black/20 sm:rounded-[24px]">
          <div className="flex flex-col gap-3 border-b border-border/60 px-3.5 py-3.5 sm:flex-row sm:items-start sm:justify-between sm:px-6 sm:py-4">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">维护您的个人昵称与基本登录账户信息。</p>
            </div>
