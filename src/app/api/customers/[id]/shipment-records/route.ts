...
export async function GET(
  ...
  try {
    ...
    const records = orders.flatMap((order) => {
      ...
      if (!isSameCustomerAddress(customer, {
      ...
      return [{
        ...
        trackingNumbers: Array.from(new Set(
          items.map((item) => String(item.tracking?.trackingNumber || "").trim()).filter(Boolean)
        )),
        logisticsNames: Array.from(new Set(
          items.map((item) => String(item.tracking?.logisticsName || "").trim()).filter(Boolean)
        )),
        items,
      }];
    });
    ...
    const productStats = Array.from(productStatsMap.values())
      .sort((a, b) => b.shipmentCount - a.shipmentCount || b.totalQuantity - a.totalQuantity || a.name.localeCompare(b.name, "zh-CN"));
    ...
    return NextResponse.json({ error: "获取客户进货记录失败" }, { status: 500 });
  }
}
