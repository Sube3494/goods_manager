...
import { getTTLockDetailByUserId, findAuthorizedTTLockUserId } from "@/lib/ttlock";
...
export async function GET(
  ...
  try {
    ...
    const result = await getTTLockDetailByUserId(userId, lockId);
    return NextResponse.json(result);
  } catch (error) {
    console.error("Failed to load public TTLock lock detail:", error);
    return NextResponse.json({
      error: error instanceof Error ? error.message : "Failed to load public TTLock lock detail",
    }, { status: 500 });
  }
}
